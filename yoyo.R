...
yoyo <- function(panel, id, waves, bmi, append = FALSE) {
  ...
  for (i in seq_along(ids)) {
    tryCatch({
      ...
      sub <- df[df[1] == ids[i], ]   # valid wave combinations for participant i
      ...
      wp[, 2:4] <- sapply(2:4, function(x) sub[, 3][match(wp[, x], unique(wp[, x]))]) # add BMI: (4, 1, 3, 4) -> (4, 21.7, 22.1, 20.2)
      ...
      if (length(yy) > 0) {
        ...
        wp <- unique(wp[, c(1, 5)])
        apply(wp, 1, function(r) df[which(df[, 2] == r[1] & df[, 1] == ids[i]), 4])
      }
    }, error = function(e){})
  }
  if (append == TRUE)
  ...
  if(exists("warn"))
    cat("\n Some ids have multiple measures per wave: only first values are used.\n")
  return(df)
}
